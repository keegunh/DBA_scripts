...
cd C:\Users\80517\Desktop\daily_check\backup_log
...
$Successful_Total = (Select-String -Path STATUS-CHECK-${TodayUTC}.txt -Pattern "SUCCESSFUL" -AllMatches).Matches.Count
$Instance_Total = (Select-String -Path STATUS-CHECK-${TodayUTC}.txt -Pattern "csql-" -AllMatches).Matches.Count

if ( $Successful_Total -eq $Instance_Total ) {
