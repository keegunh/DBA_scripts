...
CREATE OR REPLACE PROCEDURE MIG_USER.PR_MIG_INSERT
(
...
BEGIN  
    ...
    UPDATE MIG_USER.MSJ_MIG_JOB_LOG SET MIG_START_DT = SYSDATE WHERE TABLE_NAME = V_TABLE_NAME AND MIG_START_DT IS NULL;
    COMMIT;
    --DBMS_OUTPUT.PUT_LINE('INSERT TABLE_NAME:'||V_TABLE_NAME);
    EXECUTE IMMEDIATE V_MIG_SQL ;
    MIG_INSERT_COUNT := SQL%ROWCOUNT;
    MIG_RESULT       := SQLERRM ;
    UPDATE MIG_USER.MSJ_MIG_JOB_LOG SET MIG_END_DT = SYSDATE,MIG_COUNT=NVL(MIG_COUNT,0) + MIG_INSERT_COUNT,JOB_RESULT='SUCCESS' WHERE TABLE_NAME = V_TABLE_NAME;
    COMMIT;
EXCEPTION
   WHEN OTHERS THEN
    ROLLBACK;
    MIG_RESULT   :=  SQLERRM(SQLCODE) ;
    UPDATE MIG_USER.MSJ_MIG_JOB_LOG SET MIG_END_DT = SYSDATE,MIG_COUNT=-999,JOB_RESULT='FAIL:'||MIG_RESULT WHERE TABLE_NAME = V_TABLE_NAME;
    COMMIT;
END PR_MIG_INSERT;
/