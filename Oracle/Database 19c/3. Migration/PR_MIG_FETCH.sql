...
CREATE OR REPLACE PROCEDURE MIG_USER.PR_MIG_FETCH
(
  I_INST_ID NUMBER
)  
IS
  ...
  FROM (    
        SELECT /*+ LEADING(B A C) */
          ...
          FROM DBA_TAB_COLUMNS A,MIG_USER.MSJ_MIG_TABLE B,MIG_USER.MSJ_MIG_RULE C
         WHERE A.OWNER ='ADM_USER'
...
BEGIN  
  OPEN CUR1;
      LOOP
          ...
          MIG_USER.PR_MIG_INSERT(MIG_TABLE_NAME,MIG_SQL);
          --EXIT WHEN CUR1%NOTFOUND;
      END LOOP;
  CLOSE CUR1;
