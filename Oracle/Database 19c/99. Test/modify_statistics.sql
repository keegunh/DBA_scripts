-- ################ 통계 정보 생성 테스트 데이터 생성 ################ 
DROP TABLE DBA_USER.ZZ_TB_STATTEST_TEST PURGE;
CREATE TABLE DBA_USER.ZZ_TB_STATTEST_TEST
(
...
INSERT INTO ZZ_TB_STATTEST_TEST SELECT * FROM ADM_USER.TB_STATTEST@DB_LINK WHERE ROWNUM < = 10000;
COMMIT;

CREATE UNIQUE INDEX DBA_USER.PK_STATTEST
ON DBA_USER.ZZ_TB_STATTEST_TEST
(
    STACC_YM,
    BILL_ACNT_GRP_NO,
    CALL_STRT_DT,
...
LOCAL TABLESPACE TS1 PARALLEL 16 NOLOGGING;
ALTER INDEX DBA_USER.PK_STATTEST NOPARALLEL LOGGING;

ALTER TABLE DBA_USER.ZZ_TB_STATTEST_TEST
ADD CONSTRAINT PK_STATTEST PRIMARY KEY (STACC_YM,BILL_ACNT_GRP_NO,CALL_STRT_DT,EXTR_CD,STRT_TELNO,MCO_CD,MPS_FTR_CD1,CNCT_STACC_CALL_KD_DTL_CD,CNCT_STACC_CALL_KD_CD,RTNG_PREFIX_CD,KTF_RMNG_YN,CNCT_ENPR_KD_CD,IOCLL_KD_CD,SRC_ITEM_DV_CD,EXTR_KD_CD,MVNO_KD_CD);
...
-- TAB_HISTOGRAMS 조회
SELECT * FROM DBA_TAB_HISTOGRAMS WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST';

-- TABLES 통계 조회
SELECT OWNER, TABLE_NAME, NUM_ROWS, BLOCKS, EMPTY_BLOCKS, AVG_SPACE, CHAIN_CNT, AVG_ROW_LEN, AVG_SPACE_FREELIST_BLOCKS, NUM_FREELIST_BLOCKS, SAMPLE_SIZE, LAST_ANALYZED, GLOBAL_STATS FROM DBA_TABLES WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST';

-- TAB_PARTITIONS 통계 조회
SELECT TABLE_OWNER, TABLE_NAME, PARTITION_NAME, NUM_ROWS, BLOCKS, EMPTY_BLOCKS, AVG_SPACE, CHAIN_CNT, AVG_ROW_LEN, SAMPLE_SIZE, LAST_ANALYZED FROM DBA_TAB_PARTITIONS WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST';

-- TAB_SUBPARTITIONS 통계 조회
SELECT TABLE_OWNER, TABLE_NAME, PARTITION_NAME, SUBPARTITION_NAME, NUM_ROWS, BLOCKS, EMPTY_BLOCKS, AVG_SPACE, CHAIN_CNT, AVG_ROW_LEN, SAMPLE_SIZE, LAST_ANALYZED FROM DBA_TAB_SUBPARTITIONS WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST';SELECT OWNER, TABLE_NAME, COLUMN_NAME, NUM_DISTINCT, NUM_NULLS, DENSITY, LOW_VALUE, HIGH_VALUE, HISTOGRAM FROM DBA_TAB_COLUMNS WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST';
...
-- INDEXES 통계 조회
SELECT OWNER, TABLE_NAME, INDEX_NAME, BLEVEL, LEAF_BLOCKS, DISTINCT_KEYS, AVG_LEAF_BLOCKS_PER_KEY, AVG_DATA_BLOCKS_PER_KEY, CLUSTERING_FACTOR, STATUS, NUM_ROWS, SAMPLE_SIZE, LAST_ANALYZED, GLOBAL_STATS FROM DBA_INDEXES WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST';

-- IND_PARTITIONS 통계 조회
SELECT INDEX_OWNER, INDEX_NAME, BLEVEL, LEAF_BLOCKS, DISTINCT_KEYS, AVG_LEAF_BLOCKS_PER_KEY, AVG_DATA_BLOCKS_PER_KEY, CLUSTERING_FACTOR, NUM_ROWS, SAMPLE_SIZE, LAST_ANALYZED, GLOBAL_STATS FROM DBA_IND_PARTITIONS WHERE INDEX_NAME IN (SELECT INDEX_NAME FROM DBA_INDEXES WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST');

-- IND_SUBPARTITIONS 통계 조회
SELECT INDEX_OWNER, INDEX_NAME, BLEVEL, LEAF_BLOCKS, DISTINCT_KEYS, AVG_LEAF_BLOCKS_PER_KEY, AVG_DATA_BLOCKS_PER_KEY, CLUSTERING_FACTOR, NUM_ROWS, SAMPLE_SIZE, LAST_ANALYZED, GLOBAL_STATS FROM DBA_IND_SUBPARTITIONS WHERE INDEX_NAME IN (SELECT INDEX_NAME FROM DBA_INDEXES WHERE TABLE_NAME = 'ZZ_TB_STATTEST_TEST');
...
-- ################ 통계 정보 카피 ################ 
-- 테이블 내 파티션 간 통계 카피
EXEC DBMS_STATS.COPY_TABLE_STATS(OWNNAME=>'DBA_USER',TABNAME=>'ZZ_TB_STATTEST_TEST', SRCPARTNAME=>'P_202108',DSTPARTNAME=>'P_202109');

-- 테이블 내 서브파티션 간 통계 카피
EXEC DBMS_STATS.COPY_TABLE_STATS(OWNNAME=>'DBA_USER',TABNAME=>'ZZ_TB_STATTEST_TEST', SRCPARTNAME=>'P_202108_SUB_00',DSTPARTNAME=>'P_202108_SUB_01');
...
-- ################ 통계 정보 삭제 ################ 
-- 테이블 통계 삭제
EXEC DBMS_STATS.DELETE_TABLE_STATS(OWNNAME=>'DBA_USER',TABNAME=>'ZZ_TB_STATTEST_TEST');
...
*/
-- 통계정보 저장 테이블 생성
EXEC DBMS_STATS.CREATE_STAT_TABLE(OWNNAME=>'DBA_USER',STATTAB=>'STATS_20220114',TBLSPACE=>'TS_DBA_USER_D01');

-- 통계정보 저장 테이블 삭제
EXEC DBMS_STATS.DROP_STAT_TABLE(OWNNAME=>'DBA_USER',STATTAB=>'STATS_20220114');

-- 대상 테이블 통계 백업
EXEC DBMS_STATS.EXPORT_TABLE_STATS(OWNNAME=>'ADM_USER',TABNAME=>'TB_STM_RTNG_GTHR',CASCADE=>TRUE,STATTAB=>'STATS_20220114',STATOWN=>'DBA_USER');
EXEC DBMS_STATS.EXPORT_TABLE_STATS(OWNNAME=>'ADM_USER',TABNAME=>'TB_STM_RTNG_GTHR_DLY',CASCADE=>TRUE,STATTAB=>'STATS_20220114',STATOWN=>'DBA_USER');

-- 대상 테이블 통계 복구
EXEC DBMS_STATS.IMPORT_TABLE_STATS(OWNNAME=>'ADM_USER',TABNAME=>'TB_STM_RTNG_GTHR',CASCADE=>TRUE,STATTAB=>'STATS_20220114',STATOWN=>'DBA_USER');
EXEC DBMS_STATS.IMPORT_TABLE_STATS(OWNNAME=>'ADM_USER',TABNAME=>'TB_STM_RTNG_GTHR_DLY',CASCADE=>TRUE,STATTAB=>'STATS_20220114',STATOWN=>'DBA_USER');

EXEC DBMS_STATS.IMPORT_TABLE_STATS(OWNNAME=>'ADM_USER',TABNAME=>'TB_STM_RTNG_GTHR',CASCADE=>TRUE,STATTAB=>'STATS_20211206',STATOWN=>'DBA_USER',PARTNAME=>'P_202109');
EXEC DBMS_STATS.IMPORT_TABLE_STATS(OWNNAME=>'ADM_USER',TABNAME=>'TB_STM_RTNG_GTHR_DLY',CASCADE=>TRUE,STATTAB=>'STATS_20211206',STATOWN=>'DBA_USER',PARTNAME=>'P_20210901');
