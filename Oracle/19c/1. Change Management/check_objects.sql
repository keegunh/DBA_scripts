-- ################ COMMENTS 존재 여부 확인 ################
-- 코멘트 없는 테이블 확인
SELECT * FROM DBA_TAB_COMMENTS WHERE TABLE_TYPE < > 'VIEW' AND COMMENTS IS NULL AND OWNER IN ('') ORDER BY 1,2;

-- 코멘트 없는 컬럼 확인
SELECT * FROM DBA_COL_COMMENTS WHERE COMMENTS IS NULL AND TABLE_NAME NOT IN (SELECT TABLE_NAME FROM DBA_TAB_COMMENTS WHERE TABLE_TYPE = 'VIEW') AND OWNER IN ('') ORDER BY 1,2,3;
